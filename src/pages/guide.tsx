...
const Highlight = ({ children }: { children: ReactNode }) => (
  <b className="text-landing-goldLight">{children}</b>
);

const Card = ({ children, className = "" }: { children: ReactNode; className?: string }) => (
  <div className={`rounded-[14px] border border-white/[0.07] bg-landing-bgCard ${className}`}>
    {children}
  </div>
);
...
const Note = ({ tone, children }: { tone: "discord" | "gold"; children: ReactNode }) => (
  <div
    className={`rounded-[14px] border px-[26px] py-5 ${
      tone === "discord"
        ? "border-landing-discord/25 bg-landing-discord/[0.07]"
...
const Section = ({ id, step, title, badge, children }: SectionProps) => (
  <section id={id} className="scroll-mt-[90px]">
    <div className="mb-[22px] flex items-center gap-3.5">
      <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[9px] border border-landing-gold/30 bg-landing-gold/[0.14] text-[15px] font-bold text-landing-goldLight">
        {step}
      </span>
      <h2
      ...
      >
        {title}
        {badge && <span className="ml-2 text-sm font-normal text-landing-caption">{badge}</span>}
      </h2>
    ...
    <div className="overflow-x-hidden">
      ...
      >
        ...
        <div className="relative mx-auto max-w-[860px] text-center">
          <div className="mb-3.5 text-sm font-bold tracking-[1.5px] text-landing-gold">GUIDE</div>
          <h1
          ...
          <p className="mx-auto max-w-[620px] text-[17px] leading-[1.7] text-landing-caption">
        ...
        <div className="mx-auto flex max-w-[860px] flex-col gap-[52px] md:gap-20">
          <Section id="s1" step={1} title="GMOK이란">
            <Card className="flex flex-col gap-4 px-[30px] py-7">
              ...
              <p className="m-0 border-t border-white/[0.06] pt-4 text-[14.5px] leading-[1.8] text-landing-caption">
            ...
            <div className="flex flex-col gap-3">
              {START_STEPS.map((step, i) => (
                <Card key={step.title} className="flex gap-[18px] px-7 py-6">
                  <span className="flex h-[26px] w-[26px] shrink-0 items-center justify-center rounded-full bg-landing-discord/[0.14] text-[13px] font-bold text-landing-discord">
                    {i + 1}
                  </span>
                  <div>
          ...
          <Section id="s3" step={3} title="전적 기록하기 — 리플레이 업로드">
            <div className="flex flex-col gap-3">
              ...
              <Card className="px-[30px] py-7">
                ...
                <p className="m-0 border-t border-white/[0.06] pt-4 text-[14.5px] leading-[1.8] text-landing-gold">
          ...
          <Section id="s4" step={4} title="전적 보기">
            <p className="m-0 mb-4 text-[15.5px] leading-[1.8] text-landing-caption">
